...
def camera_callback(msg):
    ...
    dark_green = np.array([150,90,50])
    bright_green = np.array([255,150,100])
    # Threshold the HSV image to get only greenish colors
    mask = cv2.inRange(image, dark_green, bright_green)

    # Mask the image by cropping appropriately
    (height, width, depth) = image.shape
    top_level = height / 2
    bottom_level = top_level + 200
    ...
    if Moments['m00'] > 0:
        ...
        msgout.linear.x = np.clip(msgout.linear.x + 0.01, linear_speed_lower_limit, linear_speed_upper_limit)
        msgout.angular.z = np.clip((0.01 * (width / 2 - centroid_x)-0.01),-angular_speed_upper_limit, angular_speed_upper_limit)

    else:
        msgout.linear.x = np.clip(msgout.linear.x - 0.01, linear_speed_lower_limit, linear_speed_upper_limit)
        msgout.angular.z = np.clip(msgout.angular.z + 0.01, 0, angular_speed_upper_limit)
    ...
    cv2.imshow("Predator", image[:height,:,:])
    cv2.waitKey(2)
